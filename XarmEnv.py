...
class XarmRobotEnv():
    ...
    def __init__(self, config):
        ...
        self.goal_space = spaces.Box(low=np.array([0.3, -0.25, 0.02]),high=np.array([0.5, 0.25, 0.2]))
        self.startOrientation = p.getQuaternionFromEuler([0,0,0])
        self.golf_ball_start_default_pos = [0.45,0,0.1]
        # connect bullet
    ...
    def _get_obs(self):
        ...
        # distance
        distance = np.array([np.linalg.norm(self.goal-grip_pos,axis=-1)])

        # observation
        obs = np.concatenate((
                    grip_pos, grip_velp, distance
        ),axis= -1)
